...
export default function CurrencySelector({ setConvertForm, toOrFrom }) {
  ...
  if (toOrFrom === "convertFrom") {
    return (
      <div className="w-[350px] mb-4 text-xl z-[9999]">
    ...
    return (
      <div className="w-[350px] mb-4 text-xl">
        <Listbox value={selectedCode} onChange={setSelectedCode}>
...
//             leaveTo="opacity-0"
//           >
//             <Listbox.Options className="absolute z-50 mt-1 max-h-60 w-full overflow-y-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
//               {currencyCodes.map((c, index) => (
//                 <Listbox.Option
//                   key={index}
//                   className={({ active }) =>
...
//                       </span>
//                       {selectedCode ? (
//                         <span className="absolute z-50 inset-y-0 left-0 flex items-center pl-3 text-amber-600">
//                           <CheckIcon className="h-5 w-5" aria-hidden="true" />
//                         </span>
//                       ) : null}
//                     </>
